...
(define (value-of-exp exp env)
  (cases expression exp
    ...
    (letmutable-exp (vars exps body)
             (let ((vals (value-of-exps exps env)))
              (let ((refs (map (lambda (val) (ref-val (newref val))) vals)))
    ...
    (assign-exp (var exp1)
      ; var must be a var
      (let ((v (apply-env env var)) (val1 (value-of-exp exp1 env)))
        (let ((ref1 (expval->ref v)))
          (setref ref1 val1)
        )
      )
    )
...
(define (report-invalid-assign-exp exp)
  (eopl:error 'assign-exp "assign to immutable variable ~s is not allowed." exp)
  )
