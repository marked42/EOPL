...
(lazy-require
 ["environment.rkt" (extend-mul-env extend-mul-env-let* extend-env-unpack init-env extend-env apply-env extend-env-vec)]
...
(define (value-of-exp exp env)
  (cases expression exp
    ...
    (letrec-exp (p-name b-name p-body body)
                (let ((vec (make-vector 1)))
                  (let ((proc-env (extend-env-vec p-name vec env)))
                    (let ((new-proc (proc-val (procedure (list b-name) p-body proc-env))))
                      (vector-set! vec 0 new-proc)
                      (value-of-exp body proc-env)
                      ))
                  )
                )
