...
(define (value-of-exp exp env)
  (cases expression exp
    ...
    (proc-exp (first-var rest-vars body)
              (procedure (cons first-var rest-vars) body)
              )
    (traceproc-exp (first-var rest-vars body)
                   (trace-procedure (cons first-var rest-vars) body env)
                   )
    (letproc-exp (proc-name var proc-body body)
                 (let ((proc (procedure (list var) proc-body)))
    ...
    (call-exp (rator rands)
              (let ((rator-val (value-of-exp rator env)) (rand-vals (value-of-exps rands env)))
                ; dynamic scoping, passing in env when procedure is called
                (apply-procedure rator-val rand-vals env)
                )
              )
    (else 42)
    )
...
(define (apply-procedure proc1 args env)
  (cases proc proc1
    (procedure (vars body)
               (value-of-exp body (extend-mul-env vars args env))
               )
