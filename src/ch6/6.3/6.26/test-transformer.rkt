...
(lazy-require
 ["../cps-lang/formatter.rkt" (format-cps-program format-cps-exp)]
 ["../cps-lang/parser.rkt" (scan&parse)]
 ["transformer.rkt" (replace-free-variables)]
 )
...
(define (test-transform-diff-exp transform)
  ...
  (test-transform-exp transform "-((a 1), (b 2))" "(a 1 proc (var%2) (b 2 proc (var%3) -(var%2, var%3)))" "diff-exp")
  )

(define (test-transform-zero?-exp transform)
  (test-transform-exp transform "zero?(0)" "zero?(0)" "zero?-exp")
  (test-transform-exp transform "zero?((a 1))" "(a 1 proc (var%2) zero?(var%2))" "zero?-exp")
  )
...
(define (test-transform-sum-exp transform)
  (test-transform-exp transform "+((a 1), (b 2))" "(a 1 proc (var%2) (b 2 proc (var%3) +(var%2, var%3)))" "sum-exp")
  )
