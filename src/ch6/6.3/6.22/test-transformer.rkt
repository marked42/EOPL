...
(define (test-transform-diff-exp transform)
  ...
  (test-transform-exp transform "-((a 1), (b 2))" "(a 1 proc (var%2) (b 2 proc (var%3) let var%1 = -(var%2, var%3) in var%1))" "diff-exp")
  )

(define (test-transform-zero?-exp transform)
  (test-transform-exp transform "zero?(0)" "zero?(0)" "zero?-exp")
  (test-transform-exp transform "zero?((a 1))" "(a 1 proc (var%2) let var%1 = zero?(var%2) in var%1)" "zero?-exp")
  )
...
(define (test-transform-sum-exp transform)
  (test-transform-exp transform "+((a 1), (b 2))" "(a 1 proc (var%2) (b 2 proc (var%3) let var%1 = +(var%2, var%3) in var%1))" "sum-exp")
  )

(define (test-transform-if-exp transform)
  (test-transform-exp transform "if zero?(0) then 2 else 3" "if zero?(0) then let var%1 = 2 in var%1 else let var%1 = 3 in var%1" "if-exp")
...
(define (test-transform-let-exp transform)
  ...
  (test-transform-exp transform "let a = 1 in -(a, x)" "let a = 1 in let var%1 = -(a, x) in var%1" "let-exp")
