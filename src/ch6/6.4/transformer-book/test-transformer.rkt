...
(define (test-transformer transform)
  ...
  (test-transform-begin-exp transform)
  )
...
(define (test-transform-if-exp transform)
  ...
  (test-transform-exp transform "if 0 then if 1 then (p x1) else (p y1) else if 2 then (p x2) else (p y2)" "if 0 then if 1 then (p x1 proc (var%1) var%1) else (p y1 proc (var%1) var%1) else if 2 then (p x2 proc (var%1) var%1) else (p y2 proc (var%1) var%1)" "if-exp")
  )
...
(define (test-transform-begin-exp transform)
  (test-transform-exp transform "begin 1; (a 2); 3; (b 4); 5 end" "(a 2 proc (var%2) (b 4 proc (var%3) (proc (var%1) var%1 5)))" "begin-exp")
  )
