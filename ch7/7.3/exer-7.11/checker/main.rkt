  ...
  (cases expression exp
    ...
    (left-exp (exp1)
              (let ([type1 (type-of exp1 tenv)])
                (cases type type1
                  ...
                  (else (eopl:error 'left "left accepts only pair value, get ~s" exp1))
                  )
                )
              )
    (right-exp (exp1)
               (let ([type1 (type-of exp1 tenv)])
                (cases type type1
                  ...
                  (else (eopl:error 'right "right accepts only pair value, get ~s" exp1))
                  )
                )
               )
    (setleft-exp (exp1 exp2)
                 (let ([type1 (type-of exp1 tenv)] [type2 (type-of exp2 tenv)])
                    (cases type type1
                      (pair-type (ty1 ty2) (check-equal-type! ty1 type2 exp) type2)
                      (else (eopl:error 'setleft "setleft accepts only pair value, get ~s" exp1))
                      )
                   )
                 )
    (setright-exp (exp1 exp2)
                  (let ([type1 (type-of exp1 tenv)] [type2 (type-of exp2 tenv)])
                    (cases type type1
                      (pair-type (ty1 ty2) (check-equal-type! ty2 type2 exp) type2)
                      (else (eopl:error 'setright "setright accepts only pair value, get ~s" exp1))
                      )
                    )
                  )
  )
