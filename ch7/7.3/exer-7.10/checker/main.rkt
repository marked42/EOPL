    ...
    (deref-exp (exp1)
               (let ([exp1-type (type-of exp1 tenv)])
                 (cases type exp1-type
                   (ref-type (ty) ty)
                   (else (eopl:error 'type-of "deref accepts only ref val, get ~s" exp1))
                   )
                 )
               )
    (setref-exp (exp1 exp2)
                (let ([exp1-type (type-of exp1 tenv)] [exp2-type (type-of exp2 tenv)])
                  (cases type exp1-type
                    (ref-type (ty)
                              (check-equal-type! ty exp2-type exp)
                              (void-type)
                              )
                    (else (eopl:error 'type-of "setref accepts only ref val as first parameter, get ~s" exp1))
                    )
                  )
                )
    )
