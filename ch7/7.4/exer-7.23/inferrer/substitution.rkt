...
(define (apply-one-subst ty0 tvar ty1)
  (cases type ty0
    ...
    (pair-type (type1 type2)
               (pair-type (apply-one-subst type1 tvar ty1) (apply-one-subst type2 tvar ty1))
               )
    (tvar-type (sn)
...
(define (apply-subst-to-type ty subst)
  (cases type ty
    ...
    (pair-type (type1 type2)
               (pair-type (apply-subst-to-type type1 subst) (apply-subst-to-type type2 subst))
               )
    (tvar-type (sn)
