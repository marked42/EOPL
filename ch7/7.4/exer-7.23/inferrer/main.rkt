...
(define (type-of exp tenv subst)
  (cases expression exp
    ...
    (newpair-exp (exp1 exp2)
      (cases answer (type-of exp1 tenv subst)
        (an-answer (exp1-type subst1)
          (cases answer (type-of exp2 tenv subst1)
            (an-answer (exp2-type subst2)
              (an-answer (pair-type exp1-type exp2-type) subst2)
            )
          )
        )
      )
    ...
    (unpair-exp (var1 var2 exp1 body)
      (cases answer (type-of exp1 tenv subst)
        (an-answer (exp1-type subst1)
          (let* ([type1 (pair-type->type1 exp1-type)] [type2 (pair-type->type2 exp1-type)])
            (type-of body (extend-tenv var1 type1 (extend-tenv var2 type2 tenv)) subst1)
            )
        )
      )
    )
