  ...
  (cases expression exp
    ...
    (deref-exp (exp1)
               (cases answer (type-of exp1 tenv subst)
                 (an-answer (ty subst)
                            (if (ref-type? ty)
                                (an-answer (ref-type->type ty) subst)
                                (eopl:error 'deref-exp "deref accepts only ref-type, got ~s" ty)
                                )
                            )
                 )
               )
    (setref-exp (exp1 exp2)
                (cases answer (type-of exp1 tenv subst)
                  (an-answer (ty1 subst1)
                             (cases answer (type-of exp2 tenv subst1)
                               (an-answer (ty2 subst2)
                                          (an-answer (void-type) (unifier (ref-type->type ty1) ty2 subst2 exp))
                                          )
                               )
                             )
                  )
