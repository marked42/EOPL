...
(define test-cases-ref-exp
  (list
   (list "
    let a = newref(43)
      in a
    " '(ref int) "newref create ref type")

   (list "
    let a = newref(43)
      in deref(a)
    " 'int "deref unwraps ref type of int")

   (list "
    let a = newref(zero?(1))
      in deref(a)
    " 'bool "deref unwraps ref type of bool")

   (list "
     let a = newref(43)
       in let b = newref(42)
         in -(deref(a), deref(b))
     " 'int "deref retrieves value of a ref")

   (list "
     let a = newref(42)
       in setref(a, 43)
     " 'void "setref! returns void type")

   (list "
     let a = newref(42)
       in let b = setref(a, 43)
         in deref(a)
     " 'int "setref! updates value of a ref int")

   (list "
     let a = newref(42)
       in setref(a, zero?(0))
     " 'error "setref! throw error when type mismatch, set bool to (ref int)")

   (list "let a = 1 in (proc (x: refto int) deref(x) newref(a))" 'int "proc with parameter of ref type")
